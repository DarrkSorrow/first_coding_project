    ...
    def use_ability(self, hero, enemy, combat_log):
        if self.cool_down == 0:
            if hero.mana >= self.cost:
                ...
                combat_log.add(text)
                damage = enemy_mental_dodge(self.power, enemy, combat_log)
                hero.mana -= self.cost #MANACOST
                self.cool_down += 2 #COOLDOWN
                if damage > 0:
